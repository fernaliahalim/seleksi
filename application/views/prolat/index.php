<div class="content-wrapper">
	...
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1 class="m-0">List Prolat</h1>
				</div>
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="<?= base_url(); ?>home/sign_out">Home</a></li>
						<li class="breadcrumb-item active">List Prolat</li>
					</ol>
				</div>
			</div>
		</div>
...
<script>
	$(document).ready(function() {
		...
		var table = $("#tabel_data").DataTable({
			...
			"order": [],
			"ajax": {
				url: "<?= base_url(); ?>/prolat/fetch_data?y=<?= $tahun; ?>",
				type: "POST"
			}
		});
		...
		$('#btn-search').click(function() {
			var y = $('#y').val();
			location.href = "<?= base_url(); ?>prolat?y=" + y;
		});

		$('#btn-add').click(function() {
			$('.modal-title').html('Form Tambah List Prolat');
			$('#id-modal').val("");
			$('#nama-modal').val("");
		...
		$('.btn-save-add-data').click(function() {
			$.ajax({
				url: "<?= base_url(); ?>prolat/add",
				method: "post",
				data: $('#form-prolat').serialize(),
		...
		$("#tabel_data tbody").on("click", ".btn-update", function() {
			...
			$('#fungsi-modal').val($('#fungsi_prolat_' + id).val());
			$('#jenis-modal').val($('#jenis_prolat_' + id).val());
		...
		$('.btn-save-change-data').click(function() {
			$.ajax({
				url: "<?= base_url(); ?>prolat/edit_by_id",
				method: "post",
				data: $('#form-prolat').serialize(),
				success: function(data) {
					location.reload();
		...
		$('.btn-save-delete-data').click(function() {
			$.ajax({
				url: "<?= base_url(); ?>prolat/delete_by_id",
				method: "post",
				data: $('#form-prolat-delete').serialize(),
				success: function(data) {
					location.reload();
